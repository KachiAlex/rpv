"""
RPV Bible AI Search Assistant
FastAPI backend for semantic Bible verse search with conversational AI
"""
...
async def startup_event():
    ...
    try:
        ...
        logger.info("RPV Bible AI Assistant initialized successfully!")
...
async def root():
    ...
    return HealthResponse(
        status="healthy",
        message="RPV Bible AI Assistant is running",
...
async def search_bible(
    ...
    Args:
        q: Natural language query (e.g., "What does the Bible say about anxiety?")
        limit: Maximum number of results to return (1-50)
        session_id: Optional session ID for conversation tracking
    
    Returns:
        SearchResponse with relevant Bible verses and optional clarification
    """
    ...
    try:
        ...
        return SearchResponse(
            ...
            suggestions=suggestions[:3] if suggestions else None,  # Limit to 3 suggestions
...
async def handle_clarification(clarification: ClarificationResponse):
    ...
    Returns:
        Refined search results based on clarification
    """
    if search_engine is None or not search_engine.is_ready():
