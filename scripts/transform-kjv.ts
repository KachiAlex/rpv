...
function transformKjv(inputPath: string, outputPath: string) {
  const input = readJson(inputPath);
  if (!Array.isArray(input) || !input[0]?.chapters) {
    throw new Error('Unexpected KJV input schema');
  }
  ...
  const t: Translation = { id: 'kjv', name: 'KJV', books };
  const out = { translations: [t] };
  fs.mkdirSync(path.dirname(outputPath), { recursive: true });
  fs.writeFileSync(outputPath, JSON.stringify(out));
}

const inPath = process.argv[2] || path.join(process.cwd(), 'datasets', 'kjv.json');
const outPath = process.argv[3] || path.join(process.cwd(), 'public', 'translations', 'kjv.json');

transformKjv(inPath, outPath);
