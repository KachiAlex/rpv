...
export function SearchBar({ inputRef: externalInputRef }: SearchBarProps = {} as SearchBarProps) {
  ...
  const [searchType, setSearchType] = useState<'keywords' | 'phrases' | 'exact'>('keywords');
  const [selectedIndex, setSelectedIndex] = useState(-1);
  const { translations, current, loadTranslations, getTranslationsForEndUsers } = useBibleStore();
  ...
  // Force load translations on mount
  useEffect(() => {
  ...
  // Get filtered translations for end users
  const endUserTranslations = getTranslationsForEndUsers();

  const results = useMemo(() => {
    if (!query.trim() || query.length < 2) return [];
    
    const safeTranslations = Array.isArray(endUserTranslations) ? endUserTranslations.filter(t => t && t.id && t.books) : [];
    ...
    // Adjust search options based on search type
    const searchOptions = {
      limit: 500,
      caseSensitive,
      searchType, // Pass search type to service
      includeContext: searchType === 'phrases'
    };
    
    let filteredResults = searchService.searchInTranslations(searchTranslations, query, searchOptions);
  ...
  }, [query, endUserTranslations, selectedTranslation, selectedBook, caseSensitive, searchType, searchService]);
  ...
  const books = useMemo(() => {
    ...
    if (!Array.isArray(endUserTranslations)) return [];
    endUserTranslations.forEach(t => {
        ...
        <div className="absolute z-50 w-full mt-2 bg-white dark:bg-neutral-800 border rounded-lg shadow-lg p-4 space-y-3">
          <div className="mb-3 pb-3 border-b dark:border-neutral-700">
            ...
            <div className="text-xs text-neutral-600 dark:text-neutral-400 space-y-1">
              ...
              <div>• <code className="bg-neutral-100 dark:bg-neutral-700 px-1 rounded">pharisees</code> - Find specific people/places</div>
            </div>
          </div>
          <div>
            ...
            <select
              value={searchType}
              onChange={(e) => setSearchType(e.target.value as 'keywords' | 'phrases' | 'exact')}
              className="w-full rounded-md border p-2 text-sm dark:bg-neutral-700 dark:border-neutral-600 dark:text-neutral-100"
            >
              <option value="keywords">Keywords (semantic matching)</option>
              <option value="phrases">Phrases (exact text matching)</option>
          ...
          <div>
            ...
            >
              ...
              {Array.isArray(endUserTranslations) ? endUserTranslations.map(t => (
          ...
          <div ref={resultsRef} className="max-h-[500px] overflow-y-auto">
            {results.map((result, index) => (
              <div
                ...
                className={`block px-4 py-3 border-b dark:border-neutral-700 last:border-b-0 transition-colors cursor-pointer ${
                  selectedIndex === index
