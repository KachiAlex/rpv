...
export function AudioControls({
  ...
  const verseChangeCallbackRef = useRef(onVerseChange);
  const playbackCompleteCallbackRef = useRef(onPlaybackComplete);
  ...
  useEffect(() => {
    verseChangeCallbackRef.current = onVerseChange;
  }, [onVerseChange]);

  useEffect(() => {
    playbackCompleteCallbackRef.current = onPlaybackComplete;
  }, [onPlaybackComplete]);
  ...
  useEffect(() => {
    ...
    audioService.setOnVerseChange((verseNum) => {
      verseChangeCallbackRef.current?.(verseNum);
    });
  ...
  }, [audioService, options, selectedVoice]);
  ...
  const playFromVerse = useCallback(
    async (startFrom?: number) => {
      try {
        ...
        setIsPlaying(true);
        setIsPaused(false);
        await audioService.play(verses, startFrom ?? currentVerse);
        setIsPlaying(false);
        setIsPaused(false);
    ...
    [audioService, verses, currentVerse]
  );
  ...
  useEffect(() => {
    ...
    if (autoPlayHandledRef.current === autoPlaySignal) return;
    if (!verses.length) return;

    const targetVerse =
      autoPlayStartVerse && verses.some((v) => v.number === autoPlayStartVerse)
        ? autoPlayStartVerse
        : verses[0]?.number ?? currentVerse;

    autoPlayHandledRef.current = autoPlaySignal;
    playFromVerse(targetVerse);
  }, [autoPlaySignal, autoPlayStartVerse, verses, currentVerse, playFromVerse]);
