...
function AdminLoginContent() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // Give it a moment for user to be set, then check admin status
      setTimeout(async () => {
        if (signedInUser) {
          try {
            const userService = new UserService();
    ...
    <div className="min-h-screen flex items-center justify-center bg-neutral-50">
      <div className="max-w-md w-full space-y-8 p-8 bg-white rounded-xl shadow-lg border">
        ...
        <button
          ...
          disabled={isLoading}
          className="w-full rounded-md border border-neutral-300 hover:bg-neutral-50 px-4 py-2 font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
