...
export interface PendingOperation {
  id: string;
  type: 'saveTranslation' | 'mergeTranslation' | 'addOrUpdateVerse' | 'sendToProjector' | 'updateBookPublication' | 'bulkUpdateBookPublication';
  data: any;
  timestamp: number;
  retries: number;
}
...
export class OfflineQueue {
  ...
  async processQueue(
    handlers: {
      ...
      updateBookPublication?: (data: { translationId: string; bookName: string; published: boolean }) => Promise<void>;
      bulkUpdateBookPublication?: (data: { translationId: string; bookUpdates: Array<{ bookName: string; published: boolean }> }) => Promise<void>;
    }
  ): Promise<void> {
    const operations = await this.getPendingOperations();
