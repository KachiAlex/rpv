...
export class FirestoreRepository {
  ...
  async getAllTranslations(): Promise<Translation[]> {
    ...
    for (const d of querySnapshot.docs) {
      try {
        ...
        console.log('[FirestoreRepository] Loading translation metadata only:', id, name);
        ...
        for (const b of booksSnap.docs) {
          try {
            ...
            console.warn(`[FirestoreRepository] Error loading book metadata ${b.id} for translation ${id}:`, bookError);
          }
        }

        if (id && name && Array.isArray(books)) {
          console.log('[FirestoreRepository] Translation', id, 'has', books.length, 'books (metadata only)');
    ...
    console.log('[FirestoreRepository] Returning', translations.length, 'translations (metadata only)');
    return translations;
  }

  // New method to load full translation content on demand
  async getTranslationWithContent(id: string): Promise<Translation | null> {
    ...
    try {
      ...
      console.log('[FirestoreRepository] Loading full content for translation:', translationId);
      ...
      console.log('[FirestoreRepository] Loaded full content for', translationId, ':', books.length, 'books,', totalChapters, 'chapters,', totalVerses, 'verses');
  ...
  // New method to load specific book content
  async getBookContent(translationId: string, bookName: string): Promise<{ name: string; chapters: Array<{ number: number; verses: Array<{ number: number; text: string }> }> } | null> {
    ...
    try {
      console.log('[FirestoreRepository] Loading book content:', bookName, 'from translation:', translationId);
      ...
      console.log('[FirestoreRepository] Loaded book', bookName, ':', chapters.length, 'chapters,', totalVerses, 'verses');
